...
Cloudflare 可能在首次构建前已创建 Worker 和初始化变量，但尚未绑定 `DB`。此时若账号中
还没有任何 D1 数据库，`npm run deploy` 会自动创建并绑定 `omni-mail-db`，随后执行全部
建表迁移并校验结果。Worker 已存在本身不会阻止新账号首次初始化。

Worker 缺少 `DB` 但账号已有数据库时，脚本会提示恢复绑定或在配置中显式填写确认过的
`database_id`；不会猜测复用或创建替代库。显式 ID 经核验后也可补齐缺失绑定。已有绑定
类型或 ID 无效、显式 ID 冲突、无法唯一确定账户或权限不足时仍会停止。Worker 尚未创建
但已有同名 `omni-mail-db` 时也需显式指定 ID。无需删除或重建原数据库。
